...
//   const finalJulia = dogsJulia.slice(1, -1);
//   const allDogs = finalJulia.concat(dogsKate);
//   console.log(allDogs);
...
const recommendedFood = dogs.flatMap(dogs => {
  const food = Math.round(dogs.weight ** 0.75 * 28);
  dogs.recommendedFood = food;
  return food;
});
...
console.log(`Sarah's dog is eating ${sarahDog.curFood > sarahDog.recommendedFood ? 'much' : 'little'} `);


// 3. Create an array containing all owners of dogs who eat too much ('ownersEatTooMuch') and an array with all owners of dogs who eat too little ('ownersEatTooLittle').
const ownersEatTooLittle = dogs.filter(dogs => dogs.curFood < dogs.recommendedFood).flatMap(dog => dog.owners);
const ownersEatTooMuch = dogs.filter(dogs => dogs.curFood > dogs.recommendedFood).flatMap(dog => dog.owners);
...
// 6. Log to the console whether there is any dog eating an OKAY amount of food (just true or false) Being within a range 10% above and below the recommended portion means: current > (recommended * 0.90) && current < (recommended * 1.10). Basically, the current portion should be between 90% and 110% of the recommended portion.
const checkOkay = dogs => dogs.curFood > (dogs.recommendedFood * 0.90) && dogs.curFood < (dogs.recommendedFood * 1.10);
...
const dogsSorted = dogs.map(dog => dog.recommendedFood).sort((a,b) => a - b)
